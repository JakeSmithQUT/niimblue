...
const decodeBits = (dict: ArUcoDictionary, markerId: number): number[][] => {
...
export const drawArUco = (
  ...
  const grid = decodeBits(dict, markerId);
  if (grid.length === 0) {
    drawArUcoError(ctx, width, height);
    return;
  }

  const innerSize = DICTIONARIES[dict].size;
  const totalCells = innerSize + 2;
  const cellSize = Math.floor(width / totalCells);
  const markerWidth = cellSize * totalCells;

  if (cellSize < 1 || markerWidth > width) {
    drawArUcoError(ctx, width, height);
    return;
  }
...
const drawArUcoError = (ctx: CanvasRenderingContext2D, width: number, height: number): void => {
  ctx.save();
  ctx.fillStyle = "black";
...
export const arucoDictionaryCount = (dict: ArUcoDictionary): number =>
  DICTIONARIES[dict].count;
